...
final class DeprecatedOverview {
		...
		foreach ($rows as $row) {
			...
			foreach (['error','warning'] as $severity) foreach ($audit['accessibility'][$severity] ?? [] as $rule => $entries) {
				...
				foreach ($entries as $entry) {
					...
					} elseif ($overview['findings'][$severity][$rule][$key]['device'] !== $device) $overview['findings'][$severity][$rule][$key]['device'] = 'both';
				}
			}
		}
	...
	public static function settingsList(array $overview,string $language): array {
		...
		foreach (['error','warning'] as $severity) foreach ($overview['findings'][$severity] as $rule => $entries) {
			...
			foreach ($entries as $entry) {
				...
				$item = ['id'=>$id,'description'=>str_starts_with((string) ($entry['name'] ?? ''),'_') ? language__get($language,$entry['name']) : (string) ($entry['name'] ?? ''),'attributes'=>['data-selector'=>(string) ($entry['unique'] ?? '')],'items'=>[],'icons'=>[]];
